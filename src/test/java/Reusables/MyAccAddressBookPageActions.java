...
public class MyAccAddressBookPageActions extends MyAccAddressBookRepo{
	...
	/*	aBook_stateDiv.click();
		...
		js.executeScript("document.getElementsByClassName('dijitSelectLabel')[2].innerHTML='Alabama';");
		Thread.sleep(1000);
    	AddScreenshot();
	...
	public void addNewAddressforRemove() throws InterruptedException, IOException {
		// TODO Auto-generated method stub
		WaitUntilElementVisible(aBook_addNew_CTA);
		aBook_addNew_CTA.click();
		Thread.sleep(10000);
		Assert.assertTrue(aBook_submitCTA.isDisplayed());
		Assert.assertTrue(aBook_cancelCTA.isDisplayed());
...
*/
		...
		String recipient = "rnameforremove";
...
/*		JavascriptExecutor js = (JavascriptExecutor) driver;
		js.executeScript("document.getElementsByClassName('dijitSelectLabel')[0].innerHTML='rnameforremove';");*/
		
		
		aBook_addrDropdown.click();
		aBook_removeaddr.click();
		Thread.sleep(10000);
	...
	public void restrictRemoveAddress() throws IOException {
		
		WaitUntilElementVisible(aBook_remove_CTA);
		aBook_remove_CTA.click();
		Assert.assertTrue(aBook_removeAddressRestric_tooltip.isDisplayed());
		AddScreenshot();
		logs("Removal of address is restricted for the account with just one address");
	}
